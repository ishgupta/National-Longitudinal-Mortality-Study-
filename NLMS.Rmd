...
ggplot(tmp, aes(x = cause113, y = freq)) + geom_bar(stat = "identity", aes(fill = cause113 %in% c(105, 106))) + scale_fill_manual(values = c('black','red'), labels = c("Other reasons", "Suicide")) + ggtitle("Recorded loss of Lives") + labs(x = "Cause of Death", y = "Number of Lives lost") + theme(legend.title = element_blank())

rm(tmp)
```
For the Study, objective is to find out the factors driving people towards Suicide.
...
ms_data <- as.data.frame(table(suicides_11$ageGroup, suicides_11$ms))
ms_data <- spread(ms_data, Var2, Freq)
names(ms_data)[1] <- "ageGroup"

ms_data <- data.frame(ms_data[, -1], row.names = ms_data[, 1])

matplot(ms_data, type = "l", 
        xlab = "Age Group", 
        ylab = "Number of People", 
        main = "Suicides w.r.t Marital Status", lwd = 2
        )

legend(x = 7.55, y = 220, legend = colnames(ms_data), col = c(1:6), inset = 0.05, lty = c(1:6), cex = 0.5)
...
suicides_11$urban <- ifelse(is.na(suicides_11$urban),
       ifelse(suicides_11$stater %in% stater_urban_1, "Urban", "Rural"), suicides_11$urban)
...
filter(suicides_11, is.na(suicides_11$esr))
...
### Suicidal data summary #glimpse
#### Who are the people committing Suicides.
```{r}
ggplot(suicides_11, aes(x = race, y = age, col = ms, shape = sex)) + labs(x = "Race", y = "Age") + geom_jitter(alpha = 0.3) + theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45))

ggplotly()
```
Males are in majority for commiting Suicides, with White,  Married, and Never Married topping the census. We'll study the individual contribution of these factors towards deducing the Survivability of any Subject.

```{r}
ggplot(suicides_11, aes(x = incomeGroup, y = educLevel, col = isManualLaborIndustry, shape = urban)) + geom_jitter()
...
income_fit <- survfit(survObject ~ incomeGroup, data = income_data)
...
economic_fit <- survfit(survObject ~ educLevel + incomeGroup, data = economic_data)
