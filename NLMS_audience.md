...
Many of them happen due to Health reasons, or Natural Calamities, like Earthquake or Havocs, etc. But, there are also Incidents when people give up on their Life willingly. Yes, they commit SUICIDE!
What are the possible factors, that lead them to take such an Action?
Is it because of Financial Reasons, Health Issues, being Alone, or Personal Tragedies.
...
We'll jump in to find out possible reasons for people driven to commit Suicide.
...
ggplot(tmp, aes(x = cause113, y = freq)) + geom_bar(stat = "identity", aes(fill = cause113 %in% c(105, 106))) + scale_fill_manual(values = c('black','red'), labels = c("Other reasons", "Suicide"), guide = FALSE) + ggtitle("Recorded loss of Lives") + labs(x = "Cause of Death", y = "Number of Lives lost") + theme(legend.title = element_blank())
...
stats <- data_11_years %>% count(cause113) %>% filter(cause113 != 0) %>% mutate(percent_stats = round(n / sum(n) * 100, 2))

ggplot(stats, aes(x= "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = cause113 %in% c(105, 106))) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_manual("Causes of Death", values = c("sky blue", "red"), labels = c("Other reasons", "Suicide")) + coord_polar("y", start = 0)
...
rm(list = c("data_6a", "data_6b", "data_6c", "tobaccoUsers"))
...
data_11_years$educLevel <- "NA"
data_11_years$educLevel <- ifelse(data_11_years$educ %in% c(1:4), "Upto E8",
                                ifelse(data_11_years$educ %in% c(5:8), "H1 to H4",
                                       ifelse(data_11_years$educ %in% c(9:14), "C1 to C6", "NA")))

data_11_years$educLevel <- factor(data_11_years$educLevel, levels = c("NA", "Upto E8", "H1 to H4", "C1 to C6"))

table(data_11_years$educLevel)

```

```{r, include = FALSE}
#### Group Industry into manual Labor Industry and Professional
suicides_11$isManualLaborIndustry <- "Business"

suicides_11$isManualLaborIndustry <- ifelse(suicides_11$majind %in% c(1:4, 14), "Labour", "Business")

suicides_11$isManualLaborIndustry <- as.factor(suicides_11$isManualLaborIndustry)
levels(suicides_11$isManualLaborIndustry) <- c("Business", "Labour")
...
#tmp_graph <- 
ggplot(suicides_11, aes(x = hisp, y = race, shape = sex, colour = vt)) + geom_jitter(alpha = 0.5) + labs(title = "Suicides w.r.t being a Veteran") + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank())
...
Most of the Suicides have been found by people who are not a member of any Defence Forces.

```{r, eval = FALSE, include = FALSE}
#what factors have how much of the missing Values.
library(Amelia)
...
```{r, eval = FALSE, include = FALSE}
### Encode States w.r.t Codes

code <- c(11,12,13,14,15,16,21,22,23,31,32,33,34,35,41,42,43,44,45,46,47,51,52,53,54,55,56,57,58,59,61,62,63,64,71,72,73,74,81,82,83,84,85,86,87,88,91,92,93,94,95)
...
state_data <- data.frame(code, state)
...
# Look onto find some Questions for Veterans.
tmp <- suicides_11 %>% filter(vt == "Veteran" & !is.na(health)) %>% group_by(health) %>%  select(health, esr, stater, ageGroup, race, sex, ms, incomeGroup)

ggplot(tmp, aes(x = health, y = ageGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5) + labs(title = "Veteran deaths w.r.t Age, Health, Employment, and Marital Status", x = "Health Status", y = "Age") + theme(axis.text.x = element_text(angle = 45))


ggplot(tmp, aes(x = health, y = incomeGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5)  + labs(title = "Veterans w.r.t Income, Health, Employment, and Marital Status", x = "Health Status", y = "Income", color = c("Employment", "Marital Status")) + theme(axis.text.x = element_text(angle = 45))

ggplot(tmp, aes(y = ms, x = ageGroup, colour = health)) + geom_jitter() + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Age, Marital Status, Health", x = "Age", y = "Marital Status", color = "Health")

```


1. People below age of 35 years, having Excellent health, may be facing difficulties with their Intellectual Health.

Can Government authorities do something here by initiating a Counseling with the Veterans to help them out?

2. Disabled people with Poor Health also seem to be contributing towards Suicide.

Their Disability be due to any harm for their participation in any War. Could Government authorities taking better Care of them help out from losing their Lives?

3. People with Income < 25k form the majority.

Could Government increasing their Wages help them out from taking such actions?

4. Never Married, and Separated are the least amongst Veterans. 

  Could Marriages, losing the Partner or divorce be a factor leading People into some personal issues, eventually taking a toll on mental Health?
  If yes, Married/ Divorced/ Widowed people could be focused to undergo some Counseling sessions to help them out.

  

```{r, eval = FALSE, include = FALSE}
# Add introduce a Geographical plot to visualize the Data as per State of Residence of the Veterans
tmp <- merge(tmp, state_data, by.x="stater", by.y = "code")

tmp$hover <- with(tmp, paste(state, '<br>', "Race: ", race, sex, "<br>",
                           "Income Group: ", incomeGroup, "Age Group: ", ageGroup,
                           "<br>", "Health Status: ", health))
...
p <- plot_geo(tmp, locationmode = 'USA-states') %>%
  add_trace(
    z = ~race, text = ~hover, locations = ~state,
    color = ~race, colors = 'Purples'
  ) %>%
  colorbar(title = "Suicidal Reports") %>%
  layout(
    title = 'Suicides by Veterans in US',
    geo = g
  )
...
ggplot(suicides_11, aes(x = race, y = age, col = ms, shape = sex)) + labs(title = "An overview of people committing Suicides", x = "Race", y = "Age") + geom_jitter(alpha = 0.3) + theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45))

#ggplotly(tmp_graph_1)
```


Whites have been the most out of Subjects committing Suicides, evenly spread across all Age Groups.

Males are found to be in majority for committing Suicides, with White,  Married, and Never Married topping the census. We'll study the individual contribution of these factors towards deducing the Survivability of any Subject.


```{r , echo = FALSE, message = FALSE}
### Suicides w.r.t Race compared to the total population.

data_11_years$inddea <- as.factor(data_11_years$inddea)
levels(data_11_years$inddea) <- c("Alive", "Dead")

#ggplot(data_11_years, aes(x= race, y = inddea)) + geom_bar(stat = "identity")

data_11_years$race <- as.factor(data_11_years$race)
levels(data_11_years$race) <- levels(suicides_11$race)

superset <- table(data_11_years$race)
deaths <- table(data_11_years$race[data_11_years$inddea == "Dead"])
suicides <- table(suicides_11$race)

superset <- as.data.frame(superset)
colnames(superset) <- c("race", "Count")

deaths <- as.data.frame(deaths)
colnames(deaths) <- c("race", "Count")

suicides <- as.data.frame(suicides)
colnames(suicides) <- c("race", "Count")

comparisonData <- suicides
comparisonData <- merge(superset, comparisonData, by = "race")
comparisonData <- merge(comparisonData, deaths, by = "race")

colnames(comparisonData) <- c("race", "Total", "Suicides", "Deaths")

tmp <- comparisonData %>% mutate(suicides_percent = Suicides/Total * 100) %>% mutate(deaths_percent = Deaths/Total * 100)

tmp <- tmp[, -c(2:4)]

tmp_consolidated <- melt(tmp)

#tmp$updatedValue <- 100
#tmp$updatedValue[tmp$variable == "Suicides"] <- tmp$value

tmp_1 <- melt(tmp)

ggplot(tmp_1, aes(race, value)) + geom_bar(aes(fill = variable), position = "dodge", stat= "identity") + labs(title = "Suicides compared to Deaths per Race in %age", x = "Race", y = "%age") + theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45)) + scale_fill_discrete(labels = c("Suicides", "Deaths"))


```

The data shows that Whites are the ones committing most of the Suicides, but comparitively there is more %age of the 'American Indian or Alaskan' population committing Suicides.
 
```{r, echo = FALSE, message = FALSE, fig.width=10}
# American Indian or Alaskan committing Suicides:

tmpRace = "American Indian or Alaskan"

data_11_years$majocc <- as.factor(data_11_years$majocc)

levels(data_11_years$majocc) <- c("Business", "Professional", "Service", "Sales", "Admin", "Farming", "Construction", "Mechanical", "Production", "Transportation", "Armed Forces")

suicides_11$majocc <- as.factor(suicides_11$majocc)
levels(suicides_11$majocc) <- c("Business", "Professional", "Service", "Sales", "Admin", "Farming", "Construction", "Mechanical", "Production", "Transportation", "Armed Forces")


a <- ggplot(data_11_years[!is.na(data_11_years$majocc) & data_11_years$race != "White" & data_11_years$inddea == "Dead" & !is.na(data_11_years$race), ], aes(x = majocc, fill = race)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Deaths w.r.t Occupation, and Non-White Population", x = "Industry", y = "Incidents")

b <- ggplot(suicides_11[!is.na(suicides_11$majocc) & suicides_11$educLevel != "NA" & suicides_11$race != "White", ], aes(x = majocc, fill = race)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Suicides w.r.t Occupation, and Non-White Population", x = "Industry", y = "Incidents")

alaskanSuicides <- suicides_11 %>% filter(race == tmpRace & !is.na(majocc)) %>% select(educLevel, majocc) %>% group_by(educLevel, majocc) %>% mutate(Count = n()) %>%  unique

alaskanDeaths <- data_11_years %>% filter(race == tmpRace & !is.na(majocc) & inddea == "Dead" & !is.na(race)) %>% select(educLevel, majocc) %>% group_by(educLevel, majocc) %>% mutate(Count = n()) %>%  unique

alaskanPopulation <- data_11_years %>% filter(race == tmpRace & !is.na(majocc)) %>% select(educLevel, majocc)  %>% group_by(educLevel, majocc) %>% mutate(Count = n()) %>%  unique

alaskan <- merge(alaskanSuicides, alaskanDeaths, by = c("educLevel", "majocc"))

alaskan <- merge(alaskan, alaskanPopulation, by= c("educLevel", "majocc"))

colnames(alaskan) <- c("Education", "Occupation", "Suicides", "Deaths", "Population")

alaskan <- melt(alaskan)
colnames(alaskan) <- c("Education", "Occupation", "Data", "Count")

c <- ggplot(alaskan, aes(Occupation, Count)) + geom_bar(aes(fill = Data), position = "dodge", stat = "identity") + labs(title = "Occupation impacts") + theme(axis.text.x = element_text(angle = 45))

d <- ggplot(alaskan[alaskan$Data != "Population", ], aes(Education, Count)) + geom_bar(aes(fill = Data), position = "dodge", stat = "identity") + labs (title = "Education impacts")

plot_grid(a,b, ncol = 1)
plot_grid(c,d, ncol = 1)

```
There are 2 areas- Service, and Production which should be looked into for the maximum number of Deaths, and Suicides, respectively.
American Indian or Alaskan people have committed most of the Suicides in Production, and Construction related Work areas.
However, American Indians or Alaskans did not commit Suicide who worked for Business, Sales, Farming, and Armed Forces.

 
```{r, message=FALSE, echo=FALSE, fig.with = 10}
#tmp_graph_2 <- 
ggplot(suicides_11, aes(x = incomeGroup, y = educLevel, col = isManualLaborIndustry, shape = urban)) + geom_jitter() + labs(title = "Education, Income, Area of Residence, and Working in a Labor industry") + xlab("Income") + ylab("Education Level") + theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45))
...
tmp <- suicides_11 %>% filter(educLevel == "H1 to H4", incomeGroup %in% c("Less Than 25k", "Between 25k and 50k"), !is.na(majocc), !is.na(health))

ggplot(tmp, aes(x = incomeGroup, fill = race)) + geom_bar() + theme(axis.text.x = element_text(angle = 45))

ggplot(tmp, aes(x = health, fill = majocc)) + geom_bar() + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Impacts of Occupation on Health", x = "Health", y = "Incidents")

tmp <- suicides_11 %>% filter(!is.na(majocc), !is.na(educLevel))

ggplot(tmp, aes(x = majocc, fill = educLevel)) + geom_bar() + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Suicides w.r.t Occupation, and Education", x = "Industry", y = "Incidents")

```

People with High School Education of level- H1 to H4 can be found in every Profession. But, most of them losing their Lives are in Construction, and Transportation Business.

Also, people with higher Education level of C1 to C6 are least involved in Farming Business, maybe because they move to Metro cities. 
And most of them (Engineers, Scientists, Lawyers, Judicial Workers, Judges, Teachers, Musicians, Artists, Photographers, Doctors, Pharmacists).

Could this be because of Work pressure they have to bear because of an higher level of Job they are pursuing?
Could it be helpful to incorporate a Subject in the Education System, which could help people understand and manage Mental pressures?
Majority of the Subjects had Insurance subscribed from their Employer.

```{r, eval = FALSE, include = FALSE}
# comaprison of people died with their Employment and Health vitals.
ggplot(suicides_11 %>% filter(!is.na(health)), aes(x = esr)) + geom_bar(position = "dodge", aes(fill = health)) + theme(axis.text.x = element_text(angle = 45))

ggplot(suicides_11 %>% filter(!is.na(health, !is.na(esr))), aes())

data_11_years$esr <- as.factor(data_11_years$esr)
levels(data_11_years$esr) <- levels(suicides_11$esr)

data_11_years$health <- as.factor(data_11_years$health)
levels(data_11_years$health) <- levels(suicides_11$health)

tmp <- data_11_years %>% filter(inddea == "Dead", !is.na(esr), !is.na(health)) %>% select(esr, health, cause113)

ggplot(tmp, aes(x = esr)) + geom_bar(position= "dodge", aes(fill = health)) + facet_grid(. ~ cause113 %in% c(105, 106)) + theme(axis.text.x = element_text(angle = 45))
...
```{r, echo=FALSE, message = FALSE}
#ggplot(suicides_11, aes(x = ageGroup)) + geom_bar(aes(fill = ageGroup)) + theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 90)) + labs(y = "Number of Incidents") + guides(fill = FALSE)

#### Survival w.r.t Age Group
age_data <- na.omit(select(suicides_11, ageGroup, follow, event))
age_fit <- survfit(survObject ~ ageGroup, data = age_data)

ggsurvplot(data = age_data, age_fit, pval = TRUE, main = "Survival w.r.t Age", break.time.by = 500,
           risk.table = TRUE, risk.table.height = 0.5, log.rank.weights = "survdiff",
           surv.plot.height = 3, surv.scale = "percent", 
           legend.labs = levels(suicides_11$ageGroup))
...
People in age group > 75 years are found to be most Vulnerable of committing Suicides. However, Youth in age group of 25 - 34 Years, are the majority in Count of committing Suicides.

```{r, echo = FALSE, message = FALSE}
 #### Survival w.r.t. Race

#tmp <- suicides_11 %>% count(race) %>% mutate(percent_stats = round(n / sum(n) * 100, 2))

#ggplot(tmp, aes(x = "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = race)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Race") + coord_polar("y", start  = 0)
race_data = na.omit(select(suicides_11, follow, event, race))
race_fit <- survfit(Surv(race_data$follow, race_data$event) ~ race, data = race_data)

ggsurvplot(data = race_data, race_fit, pval = TRUE, main = "Survival w.r.t Race",
           risk.table = TRUE, risk.table.height = 0.5,
           surv.plot.height = 3,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$race))

```

People of Race = Other Non- White are most vulnerable towards committing Suicides, but Suicides appear to be in majority among the Whites.

```{r, echo=FALSE, message = FALSE}
#### Survival w.r.t Gender

#ggplot(suicides_11 %>% count(sex) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = sex)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Gender")
...
ggsurvplot(data = gender_data, gender_fit, pval = TRUE, break.time.by = 500, main = "Survival w.r.t Gender",
           surv.plot.height = 2.50,
           risk.table = TRUE, risk.table.height = 0.5,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$sex))

```

Males or Females are equally vulnerable towards committing Suicide, but Males outnumber Females for the Stats.

```{r, echo=FALSE, message = FALSE}
#### Survival w.r.t Marital Status

#ggplot(suicides_11 %>% count(ms) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = "percent_stats")) + geom_bar(stat = "identity", aes(fill = ms)) + coord_polar("y", start = 0) + scale_fill_brewer("Marital Status") + theme(axis.title.x = element_blank(), axis.title.y = element_blank())
...
ggsurvplot(data = ms_data, ms_fit, pval = TRUE, break.time.by = 1000,
           surv.plot.height = 3,
           risk.table = TRUE, risk.table.height = 0.5,
           log.rank.weights = "survdiff",
           main = "Survival w.r.t Marital Status",
           legend.labs = levels(suicides_11$ms)
           )

```

Married people outnumber all others, but Widowed are most Vulnerable.

```{r, echo = FALSE, message = FALSE}
#### Survival w.r.t. Inflation Adjusted Income.
income_data <- na.omit(select(suicides_11, follow, event, incomeGroup))

#ggplot(income_data %>% count(incomeGroup) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = incomeGroup)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Income")

income_fit <- survfit(Surv(income_data$follow, income_data$event) ~ incomeGroup, data = income_data)

ggsurvplot(data = income_data, income_fit, pval = TRUE, main = "Survival w.r.t Income",
           risk.table = TRUE, risk.table.height = 0.5,
           surv.plot.height = 3,
           log.rank.weights = "survdiff", 
           legend.labs = c("Less Than 25k", "Between 25k and 50k", "More than 75k"))


```

People with Income less than 25k, are the most Vulnerable amongst the Working population.

```{r, echo = FALSE, message = FALSE}
#### Survival w.r.t Education
educ_data <- na.omit(select(suicides_11, follow, event, educLevel))

#ggplot(educ_data %>% count(educLevel) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = percent_stats)) + geom_bar(stat= "identity", aes(fill = educLevel)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Education")

education_fit <- survfit(survObject ~ educLevel, data = educ_data)

ggsurvplot(data = educ_data, education_fit, pval = TRUE, main = "Survival w.r.t Highest Education Completed",
           risk.table = TRUE, risk.table.height = 0.5,
           surv.plot.height = 3,
           log.rank.weights = "survdiff", legend.labs = levels(suicides_11$educLevel))
```

Uneducated people seem to be People who have only completed the Elementary Education seems to be most Vulnerable towards committing Suicides.
However, majority is among the people attending High Schools.
...
ggsurvplot(rel_fit, pval = TRUE, main = "Survival w.r.t Relationship of Subject with other people in Household",
           ...
           surv.plot.height = 3,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$reltrf))
...
ggsurvplot(labor_fit, pval = TRUE, main = "Survival w.r.t Being involved in Manual Labor Industry",
           ...
           surv.plot.height = 3, 
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$isManualLaborIndustry))

#Majority of the subjects are found to be working in an Industry not needing Manual Labour.
#Vulnerability of both the groups, however, is similar.
...
#ggplot(esr_data %>% count(esr) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = esr)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +scale_fill_brewer("Employed?")

employment_fit <- survfit(Surv(esr_data$follow, esr_data$event) ~ esr, data = esr_data)

ggsurvplot(data = esr_data, employment_fit, pval = TRUE, main = "Survival w.r.t Employment",
           risk.table = FALSE, risk.table.height = 0.5,
           surv.plot.height = 3,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$esr))

```

Unemployed people tend to be most vulnerable towards Suicide, but are outnumbered by Employed for records.
...
#ggplot(hisp_data %>% count(hisp) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x= " ", y = percent_stats)) + geom_bar(stat="identity", aes(fill = hisp)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Hispanic?")

hisp_fit <- survfit(survObject ~ hisp, data = hisp_data)

ggsurvplot(data = hisp_data, hisp_fit, pval = TRUE, main = "Survival w.r.t Being Hispanic",
           risk.table = TRUE, risk.table.height = 0.5,
           surv.plot.height = 3,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$hisp))
# Other Hispanics being most vulnerable are outnumbered by Non- Hispanics.
```
...
insurance_data$insured <- "Not Insured"
insurance_data$insured <- ifelse(insurance_data$hitype == 0, "Not Insured", "Insured")

insurance_data$insured <- as.factor(insurance_data$insured)
levels(insurance_data$insured) <- c("Not Insured", "Insured")

insurance_fit <- survfit(Surv(insurance_data$follow, insurance_data$event) ~ insured, data = insurance_data)

ggsurvplot(data = insurance_data, insurance_fit, pval = TRUE, main = "Survival w.r.t Type of Insurance",
           surv.plot.height = 3, surv.scale = "percent",
           risk.table = TRUE, risk.table.height = 0.5,
           log.rank.weights = "survdiff", legend.labs = levels(insurance_data$insured))

#People with Insurance are somewhat less resilient against Suicides, but are outnumbered by people not Insured.
```
...
#ggplot(urban_data %>% count(urban) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x="", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = urban)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Living in Urban area?")

urban_fit <- survfit(survObject ~ urban, data = urban_data)

ggsurvplot(data = urban_data, urban_fit, surv.plot.height = 3, surv.scale = "percent", 
           main = "Survival w.r.t. Living in Rural or Urban area", 
           risk.table = TRUE, risk.table.height = 0.5, 
           log.rank.weights = "survdiff", legend.labs = levels(suicides_11$urban), pval = TRUE)
```

People living in Urban areas outnumber Rural population for Suicides.
...
tmp <- suicides_11
tmp$tenure <- as.character(tmp$tenure)
#tmp %>% filter(ageGroup == "Under 12 Years" & race == "White" & citizen == "Born Abroad") %>% group_by(citizen) %>% summarise(tenure = count(tenure))
...
#median(as.integer(tenure_data[tenure_data$ms == "Married" & tenure_data$race == "White" & tenure_data$citizen == "Born Abroad", ]), na.rm = TRUE)
...
#ggplot(tenure_data %>% filter(!is.na(tenure)) %>% count(tenure) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x = "", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = tenure)) + coord_polar("y", start = 0) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) + scale_fill_brewer("Own House?")

tenure_fit <- survfit(survObject ~ tenure, data = tenure_data)

ggsurvplot(data = tenure_data, tenure_fit, surv.plot.height = 3, surv.scale = "percent", 
           main = "Survival w.r.t. type of House Ownership", 
           risk.table = TRUE, risk.table.height = 0.5, 
           log.rank.weights = "survdiff")

#Subjects living in their Own Houses have been found in majority, whereas all are equally vulnerable towards committing the Act.
```
...
#ggplot(citizen_data %>% count(citizen) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x="", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = citizen)) + coord_polar("y", start = 0) + scale_fill_brewer("Citizenship") + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) 

ggsurvplot(data = citizen_data, citizen_fit, pval = TRUE, log.rank.weights = "survdiff",
           risk.table = FALSE, risk.table.height = 0.5,
           surv.plot.height = 3, surv.scale = "percent",
           main = "Survival w.r.t Citizenship", 
           legend.labs = levels(citizen_data$citizen))
```

People born in US are less resilient against Suicide, and are outnumbered by people Born Abroad.
...
#ggplot(health_data %>% filter(!is.na(health)) %>% count(health) %>% mutate(percent_stats = round(n / sum(n) * 100, 2)), aes(x="", y = percent_stats)) + geom_bar(stat = "identity", aes(fill = health)) + coord_polar("y", start = 0) + scale_fill_brewer("Health") + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) 

ggsurvplot(data = health_data, health_fit, surv.plot.height = 3, pval = TRUE, main = "Survival w.r.t. Health Status",
           risk.table = TRUE, risk.table.height = 0.5,
           log.rank.weights = "survdiff", 
           legend.labs = levels(suicides_11$health)
           )
```

People with average health are least resilient but are outnumbered by people with Excellent Health.
...
ggsurvplot(data = vet_data, veteran_fit, pval = TRUE, break.time.by = 500, log.rank.weights = "survdiff",
           risk.table = TRUE, risk.table.height = 0.5,
           surv.plot.height = 2.5, surv.scale = "percent",
           main = "Survival w.r.t being a Veteran", legend.labs = levels(suicides_11$vt), risk.table.col = "strata", risk.table.fontsize = 4, ggtheme = theme_bw())

#People who are not linked to any Defence Forces are the majority among people committing Suicides.
```
...
ggsurvplot(data = tmp_data, ms_gender_fit, pval = TRUE, main = "Survival w.r.t Marital Status and Gender", break.time.by = 500, risk.table = TRUE, risk.table.height = 2, log.rank.weights = "survdiff", surv.plot.height = 0, surv.scale = "percent", risk.table.col = "strata", risk.table.fontsize = 4, palette = "Dark2", ggtheme = theme_bw())

#merge(data.frame(A = suicides_11$ms), data.frame(B = suicides_11$sex), data.frame(C = suicides_11$race), all = TRUE)
...
ggsurvplot(economic_fit, pval = TRUE, main = "Survival w.r.t Highest Degree, Income", break.time.by = 500, surv.plot.height = 4, surv.scale = "percent", risk.table.col = "strata", palette = "Dark2", ggtheme = theme_bw(), font.legend = c(8, "plain", "black"))

ggsurvplot(economic_fit, pval = TRUE, main = "Survival w.r.t Highest Degree, Income", break.time.by = 500,
           risk.table = TRUE, risk.table.height = 1, log.rank.weights = "survdiff",
           surv.plot.height = 0, surv.scale = "percent", risk.table.col = "strata", risk.table.fontsize = 4, palette = "Dark2", ggtheme = theme_bw())

```

For people with any Education level- most occurences are in Lowest income group. Along with, there are comparative occurences in the next income group of 25k to 50k for people with Education level C1 to C6.
...
tmp_labels <- unique(paste(demographics_data$hisp, demographics_data$citizen, sep = ", "))

ggsurvplot(data = demographics_data, demographics_fit, pval = TRUE, main = "Survival w.r.t Hispanic, and Citizen", break.time.by = 500, surv.plot.height = 4, surv.scale = "percent", risk.table.col = "strata", palette = "Dark2", ggtheme = theme_bw())

ggsurvplot(data = demographics_data, demographics_fit, pval = TRUE, main = "Survival w.r.t Hispanic, and Citizen", break.time.by = 500,
           risk.table = TRUE, risk.table.height = 1, risk.table.col = "strata", palette = "Dark2", ggtheme = theme_bw(), risk.table.y.text = TRUE, log.rank.weights = "survdiff",
...
#rm(list = c("age_fit", "race_data", "race_fit", "gender_data", "gender_fit", "ms_data", "ms_fit", "income_data", "income_fit", "educ_data", "education_fit", "rel_data", "rel_fit", "labor_data", "labor_fit", "esr_data", "employment_fit", "hisp_data", "hisp_fit", "insurance_data", "insurance_fit", "urban_data", "urban_fit", "citizen_data", "citizen_fit", "health_data", "health_fit", "tmp_graph", "tmp", "veteran_fit", "vet_data", "tenure_fit", "tenure_data", "hisp_fit", "hisp_data", "esr_data", "survObject", "suicides_11", "event", "tmp_graph_2", "comparisonData", "suicides", "deaths", "superset", "tmp_graph_1", "p", "g", "l", "hover", "state", "state_data", "code", "tmpdata", "suicides_6", "suicides_tu", "columnClasses"))
```
