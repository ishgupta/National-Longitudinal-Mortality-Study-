...
tmp_graph <- ggplot(suicides_11, aes(x = hisp, y = race, shape = sex, colour = vt)) + geom_jitter(alpha = 0.5) + labs(title = "Suicides w.r.t being a Veteran") + theme(axis.text.x = element_text(angle = 45), legend.title = element_blank(), axis.ticks.x = element_blank())
...
#aTmp <- 
  ggplot(tmp, aes(x = health, y = ageGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5) + labs(title = "Veteran deaths w.r.t Age, Health, Employment, and Marital Status") + theme(legend.title = element_text("Marital Status"))
#ggplotly(aTmp)

#bTmp <- 
  ggplot(tmp, aes(x = health, y = incomeGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5)  + labs(title = "Veteran deaths w.r.t Income, Health, Employment, and Marital Status") + theme(legend.title = element_text("Type of Work"))
#ggplotly(bTmp)

#cTmp <- 
  ggplot(tmp, aes(y = ms, x = ageGroup, colour = health)) + geom_jitter() + theme(axis.text.x = element_text(angle = 45)) + theme(legend.title = element_text("Health"))
#ggplotly(cTmp)

#rm(list = c("aTmp", "bTmp", "cTmp"))
...
Can Government authorities do something here by initiaitng a Counselling with the Veterans to help them out?

2. Disabled people with Poor Health also seem to be conrtibuting towards Suicide.

Their Disability be due to any harm for their participation in any War. Could Goverment authorities taking better Care of them help out from losing their Lives?
...
4. Never Married, and Separated are the least amongst Veterans. 
  ...
  If yes, Married/ Divorced/ Widowed people could be focussed to undergo some Counselling sessions to help them out.
...
```{r, message = FALSE, echo = FALSE}
tmp_graph_1 <- ggplot(suicides_11, aes(x = race, y = age, col = ms, shape = sex)) + labs(x = "Race", y = "Age") + geom_jitter(alpha = 0.3) + theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, hjust = ))
...
a <- ggplot(data_11_years[!is.na(data_11_years$majocc) & data_11_years$race != "White" & data_11_years$inddea == "Dead" & !is.na(data_11_years$race), ], aes(x = majocc, fill = race)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Deaths w.r.t Occupation, and Non-White Population", x = "Industry", y = "Number of Incidents")

b <- ggplot(suicides_11[!is.na(suicides_11$majocc) & suicides_11$educLevel != "NA" & suicides_11$race != "White", ], aes(x = majocc, fill = race)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Suicides w.r.t Occupation, and Non-White Population", x = "Industry", y = "Number of Incidents")
...
ggplot(tmp, aes(x = health, fill = majocc)) + geom_bar() + theme(axis.text.x = element_text(angle = 45))

ggplot(suicides_11[!is.na(suicides_11$majocc) & suicides_11$vt == "Veteran", ], aes(x = majocc, fill = educLevel)) + geom_bar() + theme(axis.text.x = element_text(angle = 45)) + labs(title = "Suicides w.r.t Occupation, and Education", x = "Industry", y = "Number of Incidents")
...
People in age group > 75 years are found to be most Vulnerable of commiting Suicides. However, Youth in age group of 25 - 34 Years, are the majority in Count of commiting Suicides.
...
People of Race = Other Non- White are most vulnerable towards commiting Suicides, but Suicides appear to be in majority among the Whites.
...
Males or Females are equaly vulnerable towards commiting Suicide, but Males outnumber Females for the Stats.
...
People who have only completed the Elementaty Education seems to be most Vulnerable towards committing Suicides.
...
ggsurvplot(urban_fit, surv.plot.height = 3, surv.scale = "percent", 
           ...
           risk.table = TRUE, risk.table.height = 2, 
           log.rank.weights = "survdiff", legend.labs = levels(suicides_11$urban))
```
...
ggsurvplot(veteran_fit, pval = TRUE, break.time.by = 500, log.rank.weights = "survdiff",
           ...
           main = "Survival w.r.t being a Veteran", legend.labs = levels(suicides_11$vt))

```

People who are not linked to any Defence Forces are the majority among people commiting Suicides.
...
ggsurvplot(demographics_fit, pval = TRUE, main = "Survival w.r.t Hispanic, and Citizen", break.time.by = 500, surv.plot.height = 4, surv.scale = "percent")

ggsurvplot(demographics_fit, pval = TRUE, main = "Survival w.r.t Hispanic, and Citizen", break.time.by = 500,
           risk.table = TRUE, risk.table.height = 4, log.rank.weights = "survdiff",
           surv.plot.height = 0, surv.scale = "percent")
