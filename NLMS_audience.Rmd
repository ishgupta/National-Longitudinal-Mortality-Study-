...
```{r, message = FALSE, fig.width= 7, fig.height=5, echo = FALSE}
...
aTmp <- ggplot(tmp, aes(x = health, y = ageGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5) + labs(title = "Veteran deaths w.r.t Age, Health, Employment, and Marital Status") + theme(legend.title = element_text("Marital Status"))
ggplotly(aTmp)

bTmp <- ggplot(tmp, aes(x = health, y = incomeGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5)  + labs(title = "Veteran deaths w.r.t Income, Health, Employment, and Marital Status") + theme(legend.title = element_text("Type of Work"))
ggplotly(bTmp)

cTmp <- ggplot(tmp, aes(y = ms, x = ageGroup, colour = health)) + geom_jitter() + theme(axis.text.x = element_text(angle = 45)) + theme(legend.title = element_text("Health"))
ggplotly(cTmp)
...
tmp <- suicides_11 %>% filter(educLevel == "H1 to H4", incomeGroup %in% c("Less Than 25k", "Between 25k and 50k"), !is.na(majocc))

ggplot(tmp, aes(x = health, fill = incomeGroup)) + geom_bar() + theme(axis.text.x = element_text(angle = 45))

ggplot(suicides_11[!is.na(suicides_11$majocc) & suicides_11$vt == "Veteran", ], aes(x = majocc, fill = educLevel)) + geom_bar() + theme(axis.text.x = element_text(angle = 45))

levels(suicides_11$incomeGroup)
