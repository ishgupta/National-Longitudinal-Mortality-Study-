...
ggplot(suicides_11, aes(x = hisp, y = race, shape = sex, colour = vt)) + geom_jitter(alpha = 0.5) + labs(title = "Suicides w.r.t being a Veteran") + theme(axis.text.x = element_text(angle = 45), legend.title = element_blank(), axis.ticks.x = element_blank())
...
ggplot(tmp, aes(x = health, y = ageGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5) + labs(title = "Veteran deaths w.r.t Age, Health, Employment, and Marital Status")
ggplotly()

ggplot(tmp, aes(x = health, y = incomeGroup, colour = esr, shape = ms)) + geom_jitter(alpha = 0.5)  + labs(title = "Veteran deaths w.r.t Income, Health, Employment, and Marital Status")
ggplotly()

ggplot(tmp, aes(y = ms, x = ageGroup, colour = health)) + geom_jitter() + theme(axis.text.x = element_text(angle = 45))
ggplotly()

```

1. People below age of 35 years, hvaing Excellent health may be facing difficulties with their Intellectual Health.
...
deaths <- table(data_11_years$race[data_11_years$inddea == 1])
...
comparisonData <- rbind(deaths, suicides)
comparisonData <- merge(superset, comparisonData, by = "race")
colnames(comparisonData) <- c("race", "Total", "Count", "Source")

ggplot(comparisonData, aes(y = Count, x = race, colour = data)) + geom_line()

prop.table(comparisonData$Count * 100)

tmp <- comparisonData %>% group_by(race) %>% mutate(variation = Count/Total * 100)

barplot(tmp$variation[tmp$Source == "Suicides"], ylim = c(0, 100))

barplot(tmp$variation[tmp$Source == "Suicides"])

ggplot(tmp, aes(x = race, y = variation)) + geom_bar(stat = "identity") + theme(axis.)

ggplot(tmp, aes())

barplot(comparisonData$Count)
